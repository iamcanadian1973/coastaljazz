...
function event_archive_hero() {
    ...
    if( is_tax() ) {
        $parent_heading = single_term_title( '', false );
    }
    
    // Get Hero heading, default to page title
    $heading 		        = get_field( sprintf( '%sheading', $prefix ), 'options' );
     
    $background_image	    = get_field( sprintf( '%sbackground_image', $prefix ), 'options' );
    $background_position_x	= get_field( sprintf( '%sbackground_position_x', $prefix ), 'options' );
    $background_position_y	= get_field( sprintf( '%sbackground_position_y', $prefix ), 'options' );
